...
def aStar2(grid, start_state, goal_state, heuristic=manhattan_heuristic):
    ...
    while fringe:
        ...
        if (state[0], state[1]) in visited and (best_cost is not None and cost > best_cost):
            continue
        ...
        if is_valid_move(grid, new_x, new_y):
            new_state = (new_x, new_y, direction)
            heapq.heappush(fringe, (cost + 1 + heuristic(new_state, goal_state), new_state, cost + 1, path + [(state[0], state[1])]))

        # Turn clockwise
        new_direction = direction.turn_clockwise()
        new_state = (x, y, new_direction)
        heapq.heappush(fringe, (cost + 1000 + heuristic(new_state, goal_state), new_state, cost + 1000, path + [(state[0], state[1])]))
...
def part2_heuristic(grid, start_state, state, goal_state, on_optimal: Set, cost):
    start_to_state, _ = aStar(grid, start_state, (state[0], state[1]), manhattan_heuristic)
